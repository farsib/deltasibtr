...
switch ($act) {
    case "list":
				...
				if(strlen($RepDate)!=8)//return empty 
					DSGridRender_Sql(100,"SELECT  1 from (select 1) a where false",
						"NLog_Id",
						"NLog_Id,AssignmentTo,FirstDT,LastDT,Username,SrcAddr,SrcPort,NatAddr,DstAddr,DstPort,Proto",
						"","","");
				else{
					$Tablename='deltasib_netlog.nlog'.$RepDate;
					
					if(DBSelectAsString("SHOW COLUMNS FROM $Tablename LIKE 'NatAddr'")!="")
						$sql="SELECT  NLog_Id,AssignmentTo,{$DT}DateTimeStr(FirstDT) As FirstDT,{$DT}DateTimeStr(LastDT) As LastDT,Username,INET_NtoA(SrcAddr) As SrcAddr,SrcPort,INET_NtoA(NatAddr) As NatAddr,INET_NtoA(DstAddr) As DstAddr,DstPort,Proto ".
							"From $Tablename u_p left join Hnetworkip n_l on u_p.NetlogIP_Id=n_l.NetlogIP_Id left join deltasib.Huser u on u_p.User_Id=u.User_Id ".
							"where 1 ".$sqlfilter." $SortStr ";
						else
							$sql="SELECT  NLog_Id,'-' as AssignmentTo,{$DT}DateTimeStr(FirstDT) As FirstDT,{$DT}DateTimeStr(LastDT) As LastDT,Username,INET_NtoA(SrcAddr) As SrcAddr,SrcPort,'-' As NatAddr,INET_NtoA(DstAddr) As DstAddr,DstPort,Proto ".
							"From $Tablename u_p left join deltasib.Huser u on u_p.User_Id=u.User_Id ".
							"where 1 ".$sqlfilter." $SortStr ";
					DSGridRender_Sql(100,$sql,"NLog_Id","NLog_Id,AssignmentTo,FirstDT,LastDT,Username,SrcAddr,SrcPort,NatAddr,DstAddr,DstPort,Proto","","","");
				}
    ...
    case "SelectDate":
				...
				$sql="select MID(TABLE_NAME,5,8) as Date_Id,{$DT}DATESTR(DATE_FORMAT(MID(TABLE_NAME,5,8),'%Y-%m-%d')) as Date_Label from information_schema.TABLES where TABLE_SCHEMA='deltasib_netlog' and TABLE_NAME regexp '^nlog[0-9]{8}$' order by TABLE_NAME desc";
				$options->render_sql($sql,"","Date_Id,Date_Label","","");				
