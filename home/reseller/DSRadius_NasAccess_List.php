...
window.onload = function(){
	...
	//if(Item == "" ) {return;}
	var DataTitle="دسترسی ردیوس";
	DataName="DSRadius_NasAccess_";
	// Layout   ===================================================================
	...
	Tree.attachEvent("onBeforeCheck", function(id,state){
		...
		dhtmlxAjax.get(DataName+"ListRender.php?"+un()+"&act=Apply&ParentId="+ParentId+"&Id="+id+"&state="+newstate,function (loader){
...
/*	
		...
		dhtmlxAjax.get(DataName+"ListRender.php?"+un()+"&act=do&ParentId="+ParentId+"&Id="+id+"&state="+newstate,function (loader){
			dhxLayout.cells("a").progressOff();
			response=loader.xmlDoc.responseText;
			response=CleanError(response);
			alert(response);
			var responsearray=CleanErrorToArray(response);
