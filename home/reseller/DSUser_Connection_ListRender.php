...
switch ($act) {
    case "list":
				...
				function color_rows($row){
					...
					else if($row->get_value("TerminateCause")=="Stale-User-Detect"){
							$Style="color:green;";
					}
					else if($row->get_value("TerminateCause")=="Stale-User-Delete"){
							$Style="color:red;";
					}
					else if($row->get_value("TerminateCause")=="LessDetect"){
				...
				if(($RepDate=="Last30")||($RepDate=="Last20")||($RepDate=="Last10")||($RepDate=="Last5")){
					...
					for($i=0;$i<$n;++$i){
						...
						"INET_NTOA(FramedIpAddress) as FramedIpAddress,INET_NTOA(NasIpAddress) As NasIpAddress,TerminateCause,'',NasPortType,ServiceType,FramedProtocol,".
