...
window.onload = function(){
	...
	if(RowId == "" ) {return;}
	var DataTitle="مدیریت";
	var DataName="DSServer_3_";
	var ChangeLogDataName='Server';
	var TabbarMain,TopToolbar;
	//=======TabbarMain
	var TabbarMainArray=[
					["اطلاعات","DSServer_3_Edit","70","Admin.Server.Admin.Edit",""],
					["دسترسی درگاه پرداخت","DSReseller_TerminalAccess_List","140","Admin.Server.Admin.Edit","&ParentId=1"],
					["تاریخچه پنل","DSReseller_WebHistory_List","100","Admin.Server.Admin.Edit","&ParentId=1"],
	...
	var Form1FieldHelp  ={	
							...
							NoneBlockIP:'آی پی/شبکه مثال 127.0.0.1/32,192.168.5.2/32,172.16.2.0/24',
							SessionTimeout:'تعداد ثانیه برای پایان نشست نماینده فروش در صورت عدم فعالیت'
						 };
	var Form1FieldHelpId=["PermitIP",'NoneBlockIP',"SessionTimeout"];
	...
	var Form1Str = [
		...
		{ type:"input" , name:"LastLoginDT", label:"زمان آخرین ورود :",disabled:"true", labelAlign:"left", inputWidth:150},
		{ type:"input" , name:"LastLoginIP", label:"آخرین آی پی ورود :",disabled:"true", labelAlign:"left", inputWidth:150},
		{ type:"input" , name:"PermitIP", label:"آی پی مجاز :", value:"0.0.0.0/0",maxLength:128, validate:"NotEmpty,ISPermitIp", labelAlign:"left", info:"true", inputWidth:300},
		...
		{ type: "select", name:"AutoUpdate", label: "بروزرسانی خودکار :", options:[{text: "بلی", value: "Yes",selected: true},{text: "خیر", value: "No"}],inputWidth:80,required:true},
		{ type: "select", name:"UpdateMode", label: "نوع بروزرسانی :", options:[{text: "نسخه تایید شده", value: "ApprovedVersion",selected: true},{text: "نسخه بتا", value: "BetaVersion"}],inputWidth:120,required:true},
		{ type: "select", name:"ADConnectType", label: "نوع اتصال اکتیودایکتوری :", options:[{text: "جدید", value: "New",selected: true},{text: "قدیمی", value: "Old"}],inputWidth:120,required:true},
		
		];

	var PermitView=ISPermit("Admin.Server.Admin.View");
	var PermitEdit=ISPermit("Admin.Server.Admin.Edit");
...
function LoadTabbarMain(f_TabbarMain,f_TabbarMainArray,f_RowId){
	...
	if(ISPermit(f_TabbarMainArray[i][3])){
		...
		f_TabbarMain.setContentHref(i,f_TabbarMainArray[i][1]+".php?"+un()+"&"+f_TabbarMainArray[i][4]);
	}
