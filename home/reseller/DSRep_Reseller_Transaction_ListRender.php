...
DSDebug(1,"DSRep_Reseller_Transaction_ListRender.........................................................................");
PrintInputGetPost();
if($LResellerName==""){
...
exitifnotpermit(0,"Report.Reseller.Transaction.List");
...
switch ($act) {
    case "list":
				DSDebug(0,"DSRep_Reseller_Transaction_ListRender->Filter********************************************");
				$FieldsItems=Array(
					Array("Hrt.Reseller_Transaction_Id","Hu.UserName","Hrt.User_Id"),
					Array("Hrt.Credit"),
					Array("Hrt.TransactionType","Hrt.Creator_Id","Hrt.Reseller_Id","Hrt.Relate_Reseller_Id")
				);				

				$GroupByArray=Array(
					"GroupByOn" => Array("left(SHDATESTR(Hrt.Reseller_TransactionCDT),4)","left(SHDATESTR(Hrt.Reseller_TransactionCDT),7)","left(SHDATESTR(Hrt.Reseller_TransactionCDT),10)","Hrt.Creator_Id","Hrt.Reseller_Id","Hrt.TransactionType"),
					"GroupByValue" => Array("left(SHDATESTR(Hrt.Reseller_TransactionCDT),4)","left(SHDATESTR(Hrt.Reseller_TransactionCDT),7)","left(SHDATESTR(Hrt.Reseller_TransactionCDT),10)","rc.ResellerName","r.ResellerName","Hrt.TransactionType"),
					"GroupByTitle" => Array("YearOfCreate","MonthOfCreate","DayOfCreate","Creator","Reseller","TransactionType")
				);
				...
				$WhereStr="(Hrt.TransactionType<>'Initial')";
				...
				if($req=="GetRecordCount"){
					$sql="select count(1) from Hreseller_transaction Hrt ".
						...
						"join Hreseller r on $MyResellerAccess and Hrt.Reseller_Id=r.Reseller_Id ".
						"where $WhereStr";
				...
				elseif(($req=="ShowInGrid")or($req=="SaveToFile")){		
					...
					if($ChkStatus){
						...
						$SelectStr="Hrt.Reseller_Transaction_Id,rc.ResellerName as Creator,SHDATETIMESTR(Hrt.Reseller_TransactionCDT) as TransactionCDT,Hrt.TransactionType,r.Resellername as Reseller,r_r.ResellerName as Relate_Reseller,Format(Hrt.Credit,0) as Credit,Hu.UserName as UserName";				

						$ColumnStr="Reseller_Transaction_Id,Creator,TransactionCDT,TransactionType,Reseller,Relate_Reseller,Credit,UserName";
					}
    ...
    case "SelectResellersAndOperators":
							...
							if($LISManager=='Yes')
								...
								$MyResellerAccess="(r.Reseller_Id=$LReseller_Id or $LResellerAccessAllow)";
							$sql="SELECT Reseller_Id,ResellerName From Hreseller r where $MyResellerAccess order by ResellerName Asc";
